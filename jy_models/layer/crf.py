...
#     def decode(
#         self, emissions: torch.Tensor, mask: Optional[torch.BoolTensor] = None
#     ) -> List[List[int]]:
...
#         if emissions.ndim != 3:
#             raise ValueError(
#                 f"emissions must have dimension of 3, got {emissions.ndim}"
#             )
...
#         self, emissions: torch.Tensor, mask: torch.BoolTensor
#     ) -> torch.Tensor:
#         # emissions: (seq_length, batch_size, num_tags)
#         # mask: (seq_length, batch_size)
...
class CRF(nn.Module):
    ...
    def _viterbi_decode(
        ...
        history_idx.scatter_(
            1,
            seq_ends.reshape(-1, 1, 1).expand(-1, 1, self.num_tags),
            end_tag.reshape(-1, 1, 1).expand(-1, 1, self.num_tags), )
        history_idx = history_idx.transpose(1, 0).contiguous()
        ...
        for idx in range(seq_length - 1, -1, -1):
            ...
            best_tags_arr[idx] = best_tags.data.reshape(batch_size)
    ...
    def _viterbi_decode_nbest(
        ...
        history_idx = torch.zeros(
            ...
            device=device, )
        oor_idx = torch.zeros(
            (batch_size, self.num_tags, nbest),
            dtype=torch.long,
            device=device)
        oor_tag = torch.full(
        ...
        for i in range(1, seq_length):
            ...
            next_score, indices = next_score.reshape(batch_size, -1,
                                                     self.num_tags).topk(
                                                         nbest, dim=1)
        ...
        history_idx.scatter_(
            1,
            seq_ends.reshape(-1, 1, 1, 1).expand(-1, 1, self.num_tags, nbest),
            end_tag.reshape(-1, 1, 1, nbest).expand(-1, 1, self.num_tags,
                                                    nbest), )
        history_idx = history_idx.transpose(1, 0).contiguous()
        ...
        best_tags = (torch.arange(
            nbest, dtype=torch.long, device=device).reshape(1, -1)
                     .expand(batch_size, -1))
        for idx in range(seq_length - 1, -1, -1):
            best_tags = torch.gather(history_idx[idx].reshape(batch_size, -1),
                                     1, best_tags)
            best_tags_arr[idx] = best_tags.data.reshape(batch_size,
                                                        -1) // nbest
